...
class CTx:
    ...
    def sign_transaction(self, input_index: int, private_keys: list[int], previous_script_pubkey: "Script" = None, number_pub_keys_required: int = 1, number_pub_keys_available: int = 1, redeem_script: "Script" = None, input_amount: int = 0, witness_script: "Script" = None):
        public_keys_sec = []
        for private_key in private_keys:
            public_keys_sec.append(PrivateKey(private_key).get_public_key().sec_format())

        if previous_script_pubkey is not None:
            if previous_script_pubkey == p2sh_script(hash160(p2ms_script(public_keys_sec, number_pub_keys_required, number_pub_keys_available).serialize_script()[1:])):
                ...
                data_to_sign = self.get_sig_hash_for_segwit_transaction(input_index, input_amount, redeem_script)
            elif redeem_script is not None and redeem_script.serialize_script()[1] == 0 and len(redeem_script.serialize_script()[3:]) == 32:
                data_to_sign = self.get_sig_hash_for_segwit_transaction(input_index, input_amount, witness_script)
            else:
        ...
        for index, public_key_sec in enumerate(public_keys_sec):
            if previous_script_pubkey is not None:
                if previous_script_pubkey == p2pk_script(public_key_sec):
                ...
                elif previous_script_pubkey == p2sh_script(hash160(p2ms_script(public_keys_sec, number_pub_keys_required, number_pub_keys_available).serialize_script()[1:])):
                    ...
                    script_sig = Script([redeem_script.serialize_script()[1:]])
                    self.tx_ins[input_index].witness = der_signatures_with_sighash + [public_key_sec]
                elif previous_script_pubkey.serialize_script()[1] == 0 and len(previous_script_pubkey.serialize_script()[3:]) == 32:
                    script_sig = Script([])
                    self.tx_ins[input_index].witness = [bytes(0x0)] + der_signatures_with_sighash + [redeem_script.serialize_script()[1:]]
                elif redeem_script is not None and redeem_script.serialize_script()[1] == 0 and len(redeem_script.serialize_script()[3:]) == 32:
                    script_sig = Script([redeem_script.serialize_script()[1:]])
                    self.tx_ins[input_index].witness = [bytes(0x0)] + der_signatures_with_sighash + [witness_script.serialize_script()[1:]]
                else:
