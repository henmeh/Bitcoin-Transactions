from io import BytesIO

import pytest
from src.transaction import Transaction


class TestTransaction:

    test_transaction_legacy = ""

    test_parse_parameter = [(Transaction.parse(BytesIO(bytes.fromhex(test_transaction_legacy))).version, 1),
                            (Transaction.parse(BytesIO(bytes.fromhex(test_transaction_legacy))).number_of_inputs, 1)]
    
    @pytest.mark.parametrize("parsed_tx_value, expected_tx_value", test_parse_parameter)
    def test_parse(self, parsed_tx_value, expected_tx_value):
        assert parsed_tx_value == expected_tx_value