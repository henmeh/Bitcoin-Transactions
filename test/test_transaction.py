from io import BytesIO

import pytest
from src.transaction import Transaction


class TestTransaction:

    test_transaction_legacy = ""

    test_parse_parameter = [("Transaction.parse(BytesIO(bytes.fromhex(self.test_transaction_legacy))).version", 1),
                            ("Transaction.parse(BytesIO(bytes.fromhex(self.test_transaction_legacy))).number_of_inputs", 1)]
    
    @pytest.mark.parametrize("test_input, expected", test_parse_parameter)
    def test_parse(self, test_input, expected):
        assert eval(test_input) == expected