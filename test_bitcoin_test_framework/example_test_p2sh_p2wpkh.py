...
from src.script import Script, p2pk_script, p2sh_script, p2wpkh_script
from src.crypto import hash160
from src.helper import string_to_hex

class P2SHP2WPKHTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        bob_original_script = p2wpkh_script(hash160(bob_wallet_1["wallet_public_key"].sec_format()))
        ...
        alice_p2sh_funding_tx = alice.sendtoaddress(alice_wallet_1["wallet_address"], alice_funding_amount / 100000000)
        alice_p2sh_funding_tx_serialized = alice.gettransaction(alice_p2sh_funding_tx)["hex"]
        alice_p2sh_funding_tx_parsed = CTx.parse_transaction(BytesIO(bytes.fromhex(alice_p2sh_funding_tx_serialized)))

        self.generatetoaddress(alice, 1, alice_wallet_0["wallet_address"])
        
        alice_previous_tx_id_to_spent = alice_p2sh_funding_tx
        for index, tx_out in enumerate(alice_p2sh_funding_tx_parsed.tx_outs):
        ...
        transaction_input = CTxIn(bytes.fromhex(bob_previous_tx_id_to_spent), bob_previous_tx_index_to_spent)
        ...
        bob_spending_transaction.sign_transaction(0, [bob_wallet_1["wallet_private_key_int"]], bob_script_sig, redeem_script=bob_original_script, input_amount=alice_amount_to_spent)

        bob_p2wpkh_spending_tx = bob.sendrawtransaction(bob_spending_transaction.serialize_transaction().hex())
