...
from src.script import Script, p2pk_script, p2wpkh_script
from src.crypto import hash160

class P2WPKHTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        alice_p2wpkh_funding_tx = alice.sendtoaddress(alice_wallet_1["wallet_address"], alice_funding_amount / 100000000)
        alice_p2wpkh_funding_tx_serialized = alice.gettransaction(alice_p2wpkh_funding_tx)["hex"]
        alice_p2wpkh_funding_tx_parsed = CTx.parse_transaction(BytesIO(bytes.fromhex(alice_p2wpkh_funding_tx_serialized)))

        self.generatetoaddress(alice, 1, alice_wallet_0["wallet_address"])
        
        alice_previous_tx_id_to_spent = alice_p2wpkh_funding_tx
        for index, tx_out in enumerate(alice_p2wpkh_funding_tx_parsed.tx_outs):
        ...
        alice_script_pubkey = p2wpkh_script(hash160(bob_wallet_1["wallet_public_key"].sec_format()))
        ...
        bob_previous_tx_id_to_spent = alice_p2wpkh_locking_tx
        alice_p2pk_locking_tx_serialized = bob.gettransaction(alice_p2wpkh_locking_tx)["hex"]
        ...
        bob_spending_transaction.sign_transaction(0, [bob_wallet_1["wallet_private_key_int"]], bob_script_sig, input_amount=alice_amount_to_spent)
